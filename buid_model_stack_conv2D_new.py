...
def build_model(input_shape, nb_classes, pre_model_path=None, freezen=False, freezen_layers=None):
    ...
    channel_2 = keras.layers.Conv2D(filters=128, kernel_size=1, padding='same')(input_layer)
    conv2 = keras.layers.concatenate([conv2, channel_2], axis=3)
    ...
    channel_3 = keras.layers.Conv2D(filters=128, kernel_size=1, padding='same')(input_layer)
    conv3 = keras.layers.concatenate([conv3, channel_3], axis=3)
    conv3 = keras.layers.Conv2D(filters=32, kernel_size=1, padding='same')(conv3)
    conv3 = keras.layers.normalization.BatchNormalization()(conv3)
    conv3 = keras.layers.Activation('relu')(conv3)

    conv4 = keras.layers.Conv2D(128, kernel_size=3, padding='same', )(conv3)
    channel_4 = keras.layers.Conv2D(filters=128, kernel_size=1, padding='same')(input_layer)
    conv4 = keras.layers.concatenate([conv4, channel_4], axis=3)
    conv4= keras.layers.Conv2D(filters=64, kernel_size=1, padding='same')(conv4)
